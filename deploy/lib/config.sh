...
check_env_complete() {
    ...
    local required_vars=("PORT" "ADMIN_USERNAME" "ADMIN_PASSWORD" "JWT_SECRET" "ENCRYPTION_KEY")
...
run_config_wizard() {
    ...
    # 加密密钥 (用于加密敏感数据如 GitHub Token)
    if [ -n "$ENCRYPTION_KEY" ]; then
        echo -e "当前加密密钥: ${YELLOW}${ENCRYPTION_KEY:0:16}...${NC}"
        if read_confirm "是否重新生成加密密钥? (会导致已保存的敏感数据无法解密)" "n"; then
            ENCRYPTION_KEY=$(generate_secret)
            log_success "已生成新的加密密钥"
            log_warn "注意: 已保存的 GitHub Token 等敏感数据需要重新配置"
        fi
    else
        log_info "正在生成加密密钥..."
        ENCRYPTION_KEY=$(generate_secret)
    ...
    # CORS 允许的 Origin
    log_info "CORS 配置 (WebSocket 和 API 跨域访问)"
    echo ""
    ALLOWED_ORIGINS=$(read_input "允许的 Origin (* 表示允许所有)" "${ALLOWED_ORIGINS:-*}")
    ...
    # Traefik
    if read_confirm "是否自动启动 Traefik (用于容器路由)?" "${AUTO_START_TRAEFIK:-y}"; then
    ...
    echo -e "  加密密钥:       ${YELLOW}${ENCRYPTION_KEY:0:16}...${NC}"
    echo -e "  允许的 Origin:  ${YELLOW}$ALLOWED_ORIGINS${NC}"
    echo ""
