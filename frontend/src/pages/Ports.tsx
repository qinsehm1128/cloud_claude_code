  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold">Exposed Ports</h1>
          <p className="text-sm md:text-base text-muted-foreground">View and manage all exposed container services</p>
        </div>
        <Button variant="outline" size="sm" onClick={fetchData} className="w-full md:w-auto min-h-[44px]">
                  ...
                  <div key={service.id} className="border rounded-lg p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-xs">{service.protocol}</Badge>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        <span className="text-muted-foreground">Container:</span>
                        <p className="font-medium truncate">{service.container_name || 'Unknown'}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Port:</span>
                        <p><code className="bg-muted px-2 py-0.5 rounded text-xs">{service.port}</code></p>
                      </div>
                    </div>
                    {service.name === 'VS Code' && service.code_server_domain && (
                      <div className="flex items-center gap-1">
                        <Globe className="h-3 w-3 text-green-500" />
                        <code className="bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-300 px-2 py-1 rounded text-xs break-all">
                          {service.code_server_domain}
                        </code>
                      </div>
                    )}
                    {testStatus[service.id] && testStatus[service.id] !== 'idle' && (
                      <div>
                        {testStatus[service.id] === 'success' && (
                          <Badge variant="outline" className="text-green-600 border-green-600">
                            <CheckCircle className="h-3 w-3 mr-1" />
