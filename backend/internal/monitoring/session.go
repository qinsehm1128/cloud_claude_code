...
func (s *MonitoringSession) DetectClaudeProcess() bool {
	...
	// Method: Use ps to find the main claude process (not child processes)
	// We look for the main "claude" command, excluding grep itself and short-lived child processes
	
	// Use pgrep with -o (oldest) to get the main claude process, not its children
	// Also filter out processes that are just "node" or other helpers
	cmd := []string{"sh", "-c", "pgrep -o -f '^claude\\b|/claude\\b' 2>/dev/null || pgrep -o -f 'node.*claude' 2>/dev/null || true"}
	...
	// Parse the output to get PID - pgrep -o returns only one PID (the oldest/main process)
	...
	for _, line := range lines {
		...
		// pgrep output is just the PID when using -o
		...
		if pid != "" {
			s.stateMu.Lock()
			// 只在首次检测到时打印日志，PID 变化不再打印（因为子进程 PID 会频繁变化）
			if !s.claudeDetected {
				s.claudeDetected = true
				s.claudePID = pid
				fmt.Printf("[Session] Claude Code detected in container %d, PTY %s, PID: %s\n", s.ContainerID, s.PTYSessionID, pid)
				s.broadcastStatus()
			} else if s.claudePID != pid {
				// PID 变化只更新内部状态，不打印日志（避免日志刷屏）
				s.claudePID = pid
				// 不调用 broadcastStatus，因为只是 PID 变化，状态没变
			}
			s.stateMu.Unlock()
			return true
		}
